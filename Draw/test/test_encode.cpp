...
void test_valid_encode_request_max_data()
{
    ...
    bool status = encode(&message, buffer, ArduinoCommand_size);
    PRINT_CONST("Encoded Size: ");
    PRINTLN_ENCODE_SIZE(buffer);
    PRINT_CONST("Encoded buffer: ");
    PRINTLN_LOG(buffer);
    PRINTLN_ARRAY(buffer);
    TEST_ASSERT_EQUAL(true, status);
    TEST_ASSERT_EQUAL_MEMORY("", buffer, 66);
}
...
void test_valid_encode_reply_data()
{
    ...
    bool status = encode(&reply, buffer, ArduinoReply_size);
    PRINT_CONST("Encoded Size: ");
    PRINTLN_ENCODE_SIZE(buffer);
    PRINT_CONST("Encoded buffer: ");
    PRINTLN_LOG(buffer);
    PRINTLN_ARRAY(buffer);
    TEST_ASSERT_EQUAL(true, status);
    TEST_ASSERT_EQUAL_MEMORY("\x0a\x2e\x0a\x0b\x6d\x61\x63\x20\x61\x64\x64\x72\x65\x73\x73\x12\x0f\x73\x69\x67\x6e\x61\x6c\x20\x73\x74\x72\x65\x6e\x67\x74\x68\x1a\x0e\x73\x65\x72\x76\x69\x63\x65\x20\x73\x65\x74\x20\x69\x64\x12\x05\x0d", buffer, 27);
}
#endif